...
function Test-PolicyDocuments {
    ...
    foreach ($Policy in $RequiredPolicies) {
        ...
        if (Test-Path $PolicyPath) {
            ...
            Write-Host "  [âœ“] $Policy" -ForegroundColor Green
        } else {
            $MissingPolicies += $Policy
            Write-Host "  [âœ—] $Policy" -ForegroundColor Red
        }
    }
...
function Test-AssetInventory {
    ...
    if (-not (Test-Path $AssetRegisterPath)) {
        Write-Host "  [âœ—] Asset register not found" -ForegroundColor Red
    ...
    Write-Host "  [âœ“] Asset register found" -ForegroundColor Green
...
function Test-SupplierRiskAssessment {
    ...
    if (-not (Test-Path $SupplierRegisterPath)) {
        Write-Host "  [âœ—] Supplier risk register not found" -ForegroundColor Red
    ...
    Write-Host "  [âœ“] Supplier risk register found" -ForegroundColor Green
...
function Test-DatabaseSecurityControls {
    ...
    if (-not (Test-Path $SchemaDir)) {
        Write-Host "  [âœ—] Database schema directory not found" -ForegroundColor Red
    ...
    Write-Host "  [âœ“] Database schema found" -ForegroundColor Green
...
function Test-APIRouteSecurity {
    ...
    if (-not (Test-Path $AppDir)) {
        Write-Host "  [âœ—] App directory not found" -ForegroundColor Red
    ...
    Write-Host "  [âœ“] API routes analyzed" -ForegroundColor Green
...
function New-ComplianceReport {
    ...
    $PolicyStatus = if ($PolicyCheck.Missing -eq 0) { "âœ… Complete" } else { "âš ï¸ Incomplete" }
    $AssetStatus = if ($AssetCheck.Status -eq "Present") { "âœ… Implemented" } else { "âŒ Missing" }
    $SupplierStatus = if ($SupplierCheck.Status -eq "Present") { "âœ… Implemented" } else { "âŒ Missing" }
    $DatabaseStatus = if ($DatabaseCheck.Status -eq "Present") { "âœ… Implemented" } else { "âŒ Missing" }
    $APIStatus = if ($APICheck.CoveragePercent -ge 95) { "âœ… Complete" } else { "âš ï¸ Review Needed" }
...
'@
    ...
    Write-Host "`n[âœ“] Compliance report generated: $ReportPath" -ForegroundColor Green
...
â•‘                                                               â•‘
â•‘   ISO 27001:2022 Automated Compliance Audit                   â•‘
â•‘   Union Eyes Platform                                         â•‘
â•‘                                                               â•‘

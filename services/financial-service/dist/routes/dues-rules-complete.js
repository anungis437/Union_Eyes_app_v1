...
const createDuesRuleSchema = zod_1.z.object({
    ...
    copePercentage: zod_1.z.coerce.number().default(0),
    pacAmount: zod_1.z.coerce.number().default(0),
    strikeFundPercentage: zod_1.z.coerce.number().default(0),
    // Late fees
    gracePeriodDays: zod_1.z.coerce.number().int().default(7),
    lateFeeType: zod_1.z.enum(['percentage', 'flat']).optional(),
    lateFeeAmount: zod_1.z.coerce.number().optional(),
    // Applicability
    ...
    effectiveTo: zod_1.z.coerce.date().optional(),
    // Metadata
...
router.delete('/:id', async (req, res) => {
    try {
        const { organizationId: organizationIdFromUser, tenantId: legacyTenantId, role } = req.user;
        const organizationId = organizationIdFromUser ?? legacyTenantId;
        const { id } = req.params;
...
//# sourceMappingURL=dues-rules-complete.js.map
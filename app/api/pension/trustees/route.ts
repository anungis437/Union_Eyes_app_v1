...
/**
 * GET /api/pension/trustees?trustBoardId=xxx
 * List all trustees for a trust board
 */
...
/**
 * POST /api/pension/trustees
 * Appoint new trustee to board
 */
export async function POST(request: NextRequest) {
  try {
    const { userId: userIdAuth } = await auth();
    if (!userIdAuth) {
      return NextResponse.json(
        { error: 'Unauthorized - Authentication required' },
        { status: 401 }
    ...
    // Check if user already a trustee on this board (if userId provided)
    if (userId) {
      const existing = await db
        ...
        .where(
          and(
            ...
            eq(pensionTrustees.userId, userId),

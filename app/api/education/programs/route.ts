    ...
    const programsWithStats = await Promise.all(
      programs.map(async (program) => {
        ...
        return {
          ...program,
          stats: {
            totalEnrolled: Number((stats as Record<string, any>).total_enrolled) || 0,
            activeEnrolled: Number((stats as Record<string, any>).active_enrolled) || 0,
            completed: Number((stats as Record<string, any>).completed) || 0,
            withdrawn: Number((stats as Record<string, any>).withdrawn) || 0,
            avgProgressPercentage: Number((stats as Record<string, any>).avg_progress_percentage) || 0,
          },
        };
      })
    );
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const activeCount = Number((enrollmentCheck as Record<string, any>).active_count) || 0;

...
const statusConfig: Record<CaseStatus, { label: string; icon: JSX.Element; color: string; dotColor: string }> = {
  pending: { 
...
const priorityConfig: Record<CasePriority, { label: string; color: string; icon: JSX.Element }> = {
  low: { label: "Low", color: "text-gray-600 bg-gray-100", icon: <Flag className="w-3 h-3" /> },
  medium: { label: "Medium", color: "text-blue-600 bg-blue-100", icon: <Flag className="w-3 h-3" /> },
  high: { label: "High", color: "text-orange-600 bg-orange-100", icon: <Flag className="w-3 h-3" /> },
  urgent: { label: "Urgent", color: "text-red-600 bg-red-100", icon: <AlertTriangle className="w-3 h-3" /> }
};
      ...
      >
        ...
        <div className="mb-8">
          ...
          <p className="text-lg text-gray-600">
            LRO Workbench - Review and manage member cases
          </p>
        </div>
          ...
          >
            <Card className="bg-white/80 backdrop-blur-sm border-white/50 shadow-lg hover:shadow-xl transition-shadow">
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm text-gray-600 mb-1">Urgent Cases</p>
                  ...
                  <CardContent className="p-12 text-center">
                    ...
                    <p className="text-gray-600 mb-6">
                      {searchQuery || selectedStatus !== "all" || selectedPriority !== "all"
                        ...
                        : "All cases have been processed. Great work!"}
                    </p>
                    {(searchQuery || selectedStatus !== "all" || selectedPriority !== "all") && (
                      <button
                        onClick={() => {

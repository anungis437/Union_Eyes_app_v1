...
export class ActionService {
    ...
    validateActionConfig(action, context) {
        const template = Array.from(this.templates.values()).find(t => t.type === action.type);
        if (!template) {
            return {
                valid: false,
     ...
     */
    async executeBatch(actions, context, actionHandler) {
        const results = [];
     ...
     */
    async executeParallel(actions, context, actionHandler) {

...
import type { TriggerType, WorkflowTrigger } from './WorkflowEngine';
...
export class TriggerService {
  ...
  private webhookTriggers: Map<string, WebhookTrigger> = new Map();
   ...
   */
  private async handleDocumentUpload(document: any): Promise<void> {
   ...
   */
  private async handleDocumentStatusChange(
    oldDocument: any,
    newDocument: any
  ): Promise<void> {
   ...
   */
  private async handleCaseStatusChange(oldCase: any, newCase: any): Promise<void> {
   ...
   */
  private parseSchedule(schedule?: string, cron?: string): number | null {
    if (schedule) {
   ...
   */
  private async loadWebhookTriggers(): Promise<void> {
   ...
   * Emit custom event
   */
  async emitEvent(eventType: string, event: TriggerEvent): Promise<void> {
    const listeners = this.eventListeners.get(eventType);
    if (listeners) {
  ...
  cleanup(): void {
    ...
    const timerValues = Array.from(this.scheduledTriggers.values());

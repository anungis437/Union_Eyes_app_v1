 ...
 */
export function buildSystemConstraints(): string {
  return [
    MUST_CITE_SOURCES,
    REMAIN_NEUTRAL,
    PROTECT_PRIVACY,
 ...
 */
export function detectUnmaskedPII(content: string): string[] {
...
export function maskPII(content: string): string {
  ...
  masked = masked.replace(
    /\b\d{1,5}\s+[\w\s]+(?:street|st|avenue|ave|road|rd|drive|dr)\b/gi,
    '[ADDRESS]'
  );

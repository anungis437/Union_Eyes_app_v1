 ...
 * ```
 */
export function useOrganization(options: UseOrganizationOptions): UseOrganizationReturn {
  ...
  // Create organization
  const createOrganization = useCallback(async (input: CreateOrganizationInput): Promise<Organization | null> => {
    try {
      setError(null);

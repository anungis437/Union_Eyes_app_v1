...
exports.EnterpriseRBACService = void 0;
const events_1 = require("events");
const logger_1 = require("../utils/logger");
class EnterpriseRBACService extends events_1.EventEmitter {
    constructor(auditService) {
        super();
        this.roles = new Map();
        this.roleAssignments = new Map();
        this.permissionCache = new Map();
        this.logger = new logger_1.SecurityLogger('EnterpriseRBACService');
    ...
    async checkAccess(request) {
        try {
            const evaluation = await this.hasPermission(request.userId, request.permission, request.context);
            return {
     ...
     */
    async approveRoleAssignment(assignmentId, approvedBy, approved) {
        try {
            let assignment;
            let userId;
            // Find the assignment
     ...
     */
    getEffectiveRoles(userId, context) {
        const userAssignments = this.roleAssignments.get(userId) || [];
        const activeAssignments = userAssignments.filter(a => a.isActive &&
            a.approvalStatus === 'approved' &&
            (!a.expiresAt || a.expiresAt > new Date()) &&
     ...
     */
    validateRoleInheritance(inheritsFrom, hierarchy) {
    ...
    addInheritedRoles(inheritsFrom, visited, roles) {
        for (const roleId of inheritsFrom) {
    ...
    roleHasPermission(role, permission) {
        return role.permissions.some(p => {
            ...
            // Check exact match
            if (p.resource === reqResource && p.action === reqAction)
                return true;
            // Check resource wildcard
            if (p.resource === reqResource && p.action === '*')
                return true;
            // Check action wildcard
            if (p.resource === '*' && p.action === reqAction)
                return true;
            return false;
        });
    }
    ...
    checkIpRestrictions(restrictions, userIp) {
    ...
    checkRoleConditions(conditions, context) {
...
exports.EnterpriseRBACService = EnterpriseRBACService;
//# sourceMappingURL=enterprise-rbac.service.js.map
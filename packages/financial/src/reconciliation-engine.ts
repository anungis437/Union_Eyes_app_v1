...
export interface ReconciliationInput {
  ...
  tenantId: string;
  toleranceAmount?: number; // Allow small variances (e.g., $0.01)
  tolerancePercentage?: number; // Allow percentage variance (e.g., 1%)
}
...
export class ReconciliationEngine {
  ...
  constructor(toleranceAmount: number = 0.01, tolerancePercentage: number = 0.5) {
  ...
  async reconcile(input: ReconciliationInput): Promise<ReconciliationResult> {
    ...
    const unmatchedRemittances: RemittanceRecord[] = [];
    const unmatchedTransactions: DuesTransaction[] = [...input.existingTransactions];

    // Filter transactions that haven't been matched yet
    const availableTransactions = unmatchedTransactions.filter(
      (t) => !t.remittanceId || t.remittanceId === input.remittanceId
    );
   ...
   */
  private checkMemberIdMatch(remRecord: RemittanceRecord, transaction: DuesTransaction): boolean | null {
   ...
   */
  private checkPeriodMatch(remRecord: RemittanceRecord, transaction: DuesTransaction): boolean {
    return (
      this.isSameDay(remRecord.billingPeriodStart, transaction.periodStart) &&
   ...
   */
  private checkPeriodOverlap(remRecord: RemittanceRecord, transaction: DuesTransaction): boolean {
    return (
      remRecord.billingPeriodStart <= transaction.periodEnd &&
   ...
   */
  private normalizeMemberId(id: string): string {

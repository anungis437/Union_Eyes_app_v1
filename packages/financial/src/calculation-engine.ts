...
export interface DuesRule {
  ...
  baseField?: string; // 'gross_wages', 'base_salary', 'hourly_rate'
...
export class DuesCalculationEngine {
  private readonly MAX_FORMULA_LENGTH = 500;
  private readonly SAFE_FORMULA_REGEX = /^[0-9+\-*/(). _a-z]+$/i;
  
  /**
   * Calculate dues for a single member
   */
  async calculateMemberDues(input: CalculationInput): Promise<CalculationResult> {
    const steps: string[] = [];
    ...
    try {
      ...
      if (input.existingArrears && input.existingArrears > 0) {
        lateFeeAmount = this.calculateLateFees(
          input.existingArrears,
          input.rule,
          input.dueDate,
          steps
        );
      ...
      throw new Error(`Failed to calculate dues for member ${input.memberId}: ${(error as Error).message}`);
    }
  }
   ...
   */
  private async calculateBaseDues(input: CalculationInput, steps: string[]): Promise<Decimal> {
    const { rule } = input;
   ...
   */
  private calculatePercentageBased(input: CalculationInput, steps: string[]): Decimal {
    const { rule } = input;
   ...
   */
  private calculateFlatRate(input: CalculationInput, steps: string[]): Decimal {
    const { rule } = input;
   ...
   */
  private calculateHourlyBased(input: CalculationInput, steps: string[]): Decimal {
    const { rule } = input;
   ...
   */
  private calculateTiered(input: CalculationInput, steps: string[]): Decimal {
    const { rule } = input;
   ...
   */
  private calculateFormula(input: CalculationInput, steps: string[]): Decimal {
    const { rule } = input;
   ...
   */
  private evaluateSafeFormula(formula: string): Decimal {
   ...
   */
  async batchCalculateDues(
    assignments: Array<{ member: any; assignment: MemberDuesAssignment; rule: DuesRule }>,
   ...
   */
  private calculateDuesSync(input: CalculationInput): CalculationResult {
    const steps: string[] = [];
    const errors: string[] = [];
    ...
    try {
      ...
      // Calculate total
      const totalAmount =
        baseDuesAmount + copeAmount + pacAmount + initiationFee + strikeFundAmount + lateFeeAmount;
  ...
  private calculateBaseDuesSync(input: CalculationInput, steps: string[]): number {
  ...
  private calculateFlatRateSync(input: CalculationInput, steps: string[]): number {
    const { rule } = input;
    if (!rule.flatAmount) {
  ...
  private calculatePercentageBasedSync(input: CalculationInput, steps: string[]): number {
  ...
  private calculateHourlyBasedSync(input: CalculationInput, steps: string[]): number {
  ...
  private calculateTieredSync(input: CalculationInput, steps: string[]): number {
    // Simplified - just return 0 for now
    steps.push('Tiered calculation not fully implemented');
    return 0;
  }

  private calculateFormulaSync(input: CalculationInput, steps: string[]): number {
    // Simplified - just return 0 for now

...
serve(async (req) => {
    try {
        ...
        if (!event.actionType || !event.actionCategory || !event.result) {
            return new Response(JSON.stringify({ error: 'Missing required fields: actionType, actionCategory, result' }), { status: 400, headers: { 'Content-Type': 'application/json' } });
        }
 ...
 */
async function enrichWithGeolocation(ipAddress) {
    if (!ipAddress) {
        return {};
    }
    try {
...
function calculateRiskLevel(event) {
    ...
    // High risk factors
    if (event.actionType.includes('password_') ||
    ...
    // Medium risk factors
    if (event.actionType.includes('data.modified') ||
 ...
 */
async function detectAnomalies(supabase, event, auditLog) {
    try {
 ...
 */
async function createAnomalyAlert(supabase, alert) {
    try {

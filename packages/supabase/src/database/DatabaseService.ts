...
export class DatabaseService {
   ...
   */
  async updateDocumentSearchIndex(
    documentId: string,
    title: string,
    content: string,
   ...
   */
  async generateBillingSummary(params: BillingSummaryParams): Promise<BillingSummary> {
   ...
   */
  async getLegalHolds(organizationId: string, isActive = true): Promise<LegalHold[]> {
   ...
   */
  async createLegalHold(legalHold: Omit<LegalHold, 'id' | 'created_at' | 'updated_at'>): Promise<LegalHold> {
   ...
   */
  async getDocumentClassifications(documentId: string): Promise<DocumentClassification[]> {
    const { data, error } = await this.supabase
   ...
   */
  async createDocumentClassification(
    classification: Omit<DocumentClassification, 'id' | 'created_at'>
   ...
   */
  async validateLegalDocument(documentData: Record<string, any>): Promise<boolean> {
   ...
   */
  async getDocumentSecurityOverview(organizationId: string): Promise<DocumentSecurityOverview[]> {
    const { data, error } = await this.supabase
  ...
  async createDataRetentionPolicy(
    policy: Omit<DataRetentionPolicy, 'id' | 'created_at' | 'updated_at'>
   ...
   */
  async calculateDocumentChecksum(documentContent: Uint8Array): Promise<string> {
   ...
   */
  async refreshDailyUsageStats(): Promise<void> {
...
export function useComplianceMonitoring(organizationId?: string) {
  ...
  const [securityOverview, setSecurityOverview] = useState<DocumentSecurityOverview[]>([]);

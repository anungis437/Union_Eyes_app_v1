...
export async function getCurrentUserOrganizationId(): Promise<string | null> {
  ...
  try {
    ...
    if (user?.id) {
      ...
      return (userData as unknown as User)?.organization_id || null;
    }
 ...
 */
export async function subscribeToUserChanges<T>(
  tableName: string,
  callback: (payload: {
    eventType: 'INSERT' | 'UPDATE' | 'DELETE';
    new: T | null;
 ...
 */
export async function validateTrustTransaction(
  clientId: string,
  matterId: string,
  ...
  if (transactionType === 'withdrawal' && amount > 0) {
    ...
    const trustLedger = ledger as unknown as TrustLedger;
...
export {
  ...
  getSupabaseServiceClient as supabaseService,
};

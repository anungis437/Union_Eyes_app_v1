...
export class InvitationService {
    constructor(supabaseUrl, supabaseKey, baseUrl) {
    ...
    async generateInvitationToken() {
        const size = 32;
        // Browser / Web Crypto
        if (typeof window !== 'undefined' && globalThis.crypto && typeof globalThis.crypto.getRandomValues === 'function') {
        ...
        try {
            ...
            for (let i = 0; i < fallback.length; i++)
                binary += String.fromCharCode(fallback[i]);
            const base64 = btoa(binary);
            return base64.replace(/\+/g, '-').replace(/\//g, '_').replace(/=+$/, '');
        }
    }
     ...
     */
    async createInvitation(data, invitedBy) {
        try {
...
return false;
     ...
     */
    generateEmailHTML(email, invitationUrl, organizationName, inviterName, role, daysUntilExpiration) {
        return `
<!DOCTYPE html>
<html>
<head>
...
<body>
  <div class="container">
     ...
     */
    generateEmailText(email, invitationUrl, organizationName, inviterName, role, daysUntilExpiration) {
        return `
You're Invited!

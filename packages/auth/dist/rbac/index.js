...
export class RBAC {
    ...
    static hasPermission(role, permission) {
        ...
        if (roleDefinition.inherits) {
            return roleDefinition.inherits.some(inheritedRole => this.hasPermission(inheritedRole, permission));
        }
        return false;
    }
    /**
    ...
    static getPermissions(role) {
        ...
        if (roleDefinition.inherits) {
            roleDefinition.inherits.forEach(inheritedRole => {
                this.getPermissions(inheritedRole).forEach(permission => permissions.add(permission));
            });
        }
     ...
     */
    static canPerformAction(role, action, resource) {
     ...
     */
    static filterPermissions(role, requestedPermissions) {
        const rolePermissions = this.getPermissions(role);
        return requestedPermissions.filter(permission => rolePermissions.includes(permission));
    }
}
// =========================================================================
// EXPORTS

...
export class AnomalyDetectionService {
     ...
     */
    async detectUnusualLoginPatterns(userId, firmId) {
        try {
            const anomalies = [];
            // Get user baseline
...
return {
     ...
     */
    async detectImpossibleTravel(logins) {
        if (logins.length < 2)
            return null;
        // Sort by time
        const sortedLogins = [...logins].sort((a, b) => new Date(a.attempted_at).getTime() - new Date(b.attempted_at).getTime());
     ...
     */
    async detectDataAccessAnomalies(userId, firmId) {
        try {
            const anomalies = [];
            // Get user baseline
            const baseline = await this.getUserBaseline(userId, firmId);
            ...
            if (baseline) {
                const unusualResources = Array.from(accessedResources).filter(r => !baseline.typical_resources_accessed.includes(r));
...
return {
     ...
     */
    async detectPermissionAnomalies(userId, firmId) {
        try {
            const anomalies = [];
...
return {
     ...
     */
    async detectSessionAnomalies(userId, firmId) {
        try {
            const anomalies = [];
            // Get active sessions
...
return {
     ...
     */
    async getUserBaseline(userId, firmId) {
        try {
...
return null;
     ...
     */
    async buildUserBaseline(userId, firmId) {
        try {
            ...
            logs.forEach(log => {
                ...
                loginHours.add(hour);
                if (log.city)
                    locations.add(log.city);
                if (log.ip_address)
                    ipAddresses.add(log.ip_address);
                if (log.resource_type)
                    resources.add(log.resource_type);
            });
            const baseline = {
                user_id: userId,
                firm_id: firmId,
...
return null;
     ...
     */
    async updateBaseline(userId, firmId) {
        try {
...
// FACTORY FUNCTION
// ============================================================================
export function createAnomalyDetectionService(supabaseClient) {

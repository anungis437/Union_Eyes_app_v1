...
export class SessionManagement {
   ...
   */
  async getAllSessions(limit: number = 100): Promise<SessionInfo[]> {
    try {
      const { data, error } = await this.supabase
        .from('user_sessions')
        .select(`
  ...
  private parseUserAgent(userAgent?: string): {
    ...
    const result: ReturnType<typeof this.parseUserAgent> = {};
 ...
 */
export function formatSessionDuration(startDate: Date, endDate: Date = new Date()): string {

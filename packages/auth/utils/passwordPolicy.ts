...
export interface PasswordPolicyConfig {
  ...
  preventUserInfo: boolean; // Prevent using email, name in password
  maxConsecutiveChars?: number;
  maxRepeatingChars?: number;
  historyCount?: number; // Number of previous passwords to check
  expirationDays?: number; // Password expiration (0 = never)
  minStrengthScore?: number; // Minimum strength score (0-100)
}
...
export function calculatePasswordStrength(
  ...
  if (!requirements.length) feedback.push(`Must be at least ${config.minLength} characters`);
  if (!requirements.uppercase && config.requireUppercase) feedback.push('Must include uppercase letters');
  if (!requirements.lowercase && config.requireLowercase) feedback.push('Must include lowercase letters');
  if (!requirements.numbers && config.requireNumbers) feedback.push('Must include numbers');
  if (!requirements.specialChars && config.requireSpecialChars) feedback.push('Must include special characters');
...
export function generatePolicyDescription(config: PasswordPolicyConfig): string[] {

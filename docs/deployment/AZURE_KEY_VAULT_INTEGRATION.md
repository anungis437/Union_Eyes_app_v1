...
- ✅ No keys exposed to application code
...
│  │  1. Retrieves key from Key Vault using managed identity  │  │
│  │  2. Sets key in database session variable                │  │
│  │  3. Executes PII queries                                 │  │
│  └──────────────────────────────────────────────────────────┘  │
│                              ↓                                  │
│  ┌──────────────────────────────────────────────────────────┐  │
│  │  lib/azure-keyvault.ts                                   │  │
│  │  - getEncryptionKey()                                    │  │
│  │  - setEncryptionKeyInSession(db)                         │  │
...
│  ┌──────────────────────────────────────────────────────────┐  │
│  │  Session Variable: app.encryption_key                    │  │
│  │  ↓                                                        │  │
│  │  encrypt_pii(plaintext) → uses current_setting()         │  │
│  │  decrypt_pii(ciphertext) → uses current_setting()        │  │
│  │  ↓                                                        │  │
│  │  Encrypted PII stored in members table                   │  │
│  └──────────────────────────────────────────────────────────┘  │
└─────────────────────────────────────────────────────────────────┘
```
...
4. ✅ Enable managed identity on PostgreSQL server
5. ✅ Grant Key Vault access to PostgreSQL
6. ✅ Configure audit logging
7. ✅ Run database migration (`066_azure_key_vault_integration.sql`)
8. ✅ Verify setup
...
All 22 encryption tests should pass ✅
...
export async function GET(
  ...
  try {
    ...
    console.error('Failed to retrieve member PII:', error);
...
export async function updateMemberPII(
  ...
  try {
    ...
    console.error('Failed to update member PII:', error);
...
console.log('Key Vault Access Stats:', {

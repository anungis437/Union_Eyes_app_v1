...
CREATE POLICY select_remittances ON per_capita_remittances
  ...
  USING (
    from_organization_id IN (
      ...
      OR parent_organization_id = current_setting('app.current_organization_id', TRUE)::UUID
    )
...
WITH RECURSIVE member_rollup AS (
  ...
  WHERE NOT EXISTS (
    ...
    WHERE child.parent_organization_id = o.id
  )
...
BEGIN
  ...
  SELECT parent_organization_id, per_capita_rate, remittance_day
...
BEGIN
  -- Log parent changes
  IF (TG_OP = 'UPDATE' AND 
      (OLD.parent_organization_id IS DISTINCT FROM NEW.parent_organization_id OR
    ...
    INSERT INTO organization_hierarchy_audit (
      ...
      NEW.id,
      CASE 
        WHEN OLD.parent_organization_id IS DISTINCT FROM NEW.parent_organization_id THEN 'parent_changed'
      ...
      END,
      OLD.parent_organization_id,
      NEW.parent_organization_id,
      OLD.organization_level,
      NEW.organization_level,
      OLD.per_capita_rate,
      NEW.per_capita_rate,
      (OLD.parent_organization_id IS DISTINCT FROM NEW.parent_organization_id) -- Require approval for parent changes
    );
  END IF;

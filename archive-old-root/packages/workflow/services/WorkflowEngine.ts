...
export interface WorkflowAction {
  ...
  onFailure?: string[]; // Fallback action IDs
}
...
export interface WorkflowExecution {
  ...
  actionsHistory?: ActionExecution[]; // Alias for actions to support UI components
  startedAt: Date;
  completedAt?: Date;
  duration?: number;
  error?: string;
...
export class WorkflowEngine {
  ...
  private async executeActions(
    ...
    while (actionsToExecute.length > 0) {
      ...
      if (actionExecution.status === 'completed' && action.onSuccess) {
        // Queue next actions
        const nextActions = workflow.actions.filter(a => 
          action.onSuccess!.includes(a.id) && !executedActions.has(a.id)
        );
        actionsToExecute.unshift(...nextActions);
      } else if (actionExecution.status === 'failed' && action.onFailure) {
        // Queue fallback actions
        const fallbackActions = workflow.actions.filter(a =>
          action.onFailure!.includes(a.id) && !executedActions.has(a.id)
        );
  ...
  private async executeAction(
    ...
    const retryConfig: RetryConfig = action.retryConfig || {
      maxAttempts: 3,
      delayMs: 1000,
      backoffMultiplier: 2,
      maxDelayMs: 30000,
   ...
   */
  private mapWorkflowFromDB(data: any): WorkflowDefinition {
    return {
      id: data.id,
      name: data.name,

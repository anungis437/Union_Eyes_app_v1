...
export default function GrievancesPage() {
  ...
  const statusConfig: Record<GrievanceStatus, { label: string; color: string; icon: JSX.Element }> = {
    "filed": { label: "Filed", color: "text-blue-700 bg-blue-100 border-blue-200", icon: <FileText className="w-3 h-3" /> },
    "step-1": { label: "Step 1", color: "text-purple-700 bg-purple-100 border-purple-200", icon: <Flag className="w-3 h-3" /> },
    "step-2": { label: "Step 2", color: "text-orange-700 bg-orange-100 border-orange-200", icon: <Flag className="w-3 h-3" /> },
    "step-3": { label: "Step 3", color: "text-red-700 bg-red-100 border-red-200", icon: <Flag className="w-3 h-3" /> },
    "arbitration": { label: "Arbitration", color: "text-pink-700 bg-pink-100 border-pink-200", icon: <Gavel className="w-3 h-3" /> },
    "resolved": { label: "Resolved", color: "text-green-700 bg-green-100 border-green-200", icon: <CheckCircle className="w-3 h-3" /> },
    "withdrawn": { label: "Withdrawn", color: "text-gray-700 bg-gray-100 border-gray-200", icon: <XCircle className="w-3 h-3" /> },
  };

  const priorityConfig: Record<GrievancePriority, { label: string; color: string; icon: JSX.Element }> = {
    urgent: { label: "Urgent", color: "text-red-700 bg-red-100 border-red-200", icon: <AlertTriangle className="w-4 h-4" /> },
    high: { label: "High", color: "text-orange-700 bg-orange-100 border-orange-200", icon: <Flag className="w-4 h-4" /> },
    medium: { label: "Medium", color: "text-blue-700 bg-blue-100 border-blue-200", icon: <Flag className="w-4 h-4" /> },
    low: { label: "Low", color: "text-gray-700 bg-gray-100 border-gray-200", icon: <Flag className="w-4 h-4" /> },
  };

  const timelineTypeConfig: Record<TimelineEvent["type"], { color: string; icon: JSX.Element }> = {
      ...
      <div className="p-6 md:p-10 max-w-7xl mx-auto">
        ...
        >
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-3xl font-bold text-gray-900">Grievance Process</h1>
          </div>
          <p className="text-gray-600">Manage formal grievances and arbitration cases</p>
        </motion.div>
          ...
          >
            <Card className="bg-white/80 backdrop-blur-sm border-white/50 shadow-lg">
              <CardContent className="p-6">
                <div className="flex items-start justify-between">
                  <div>
                    <p className="text-sm text-gray-600 mb-1">In Arbitration</p>
            ...
            <CardContent className="p-6">
              <div className="flex flex-col md:flex-row gap-4">
                ...
                <div className="flex-1 relative">
                  ...
                  <input
                    type="text"
                    placeholder="Search by number, title, grievant, or category..."
                    value={searchQuery}
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                ...
                {(["all", "filed", "step-1", "step-2", "step-3", "arbitration", "resolved"] as const).map((status) => (
                  ...
                  >
                    {status === "all" ? "All" : statusConfig[status as GrievanceStatus].label}
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                ...
                {(["all", "urgent", "high", "medium", "low"] as const).map((priority) => (
                  ...
                  >
                    {priority === "all" ? "All Priorities" : priorityConfig[priority as GrievancePriority].label}
                  </button>
                ))}
              </div>
            </CardContent>
        ...
        <div className="mb-4 text-sm text-gray-600">
          Showing {filteredGrievances.length} of {totalGrievances} grievances
        </div>
              ...
              >
                <Card className="bg-white/80 backdrop-blur-sm border-white/50 shadow-lg">
                  <CardContent className="p-12 text-center">
                    ...
                    <p className="text-gray-600 mb-4">No grievances match your current filters.</p>
                    <button
                      onClick={() => {
                        setSearchQuery("");
